...
lv_style_t togStyle; //toggled  style
...
lv_btn_set_style(btn, LV_BTN_STYLE_PR, &togStyle); //set the pressed style
...
void btnCreate(){
  ...
  lv_style_copy(&relStyle, &lv_style_plain);
   ...
   lv_style_copy(&togStyle, &lv_style_plain);
   togStyle.body.main_color = LV_COLOR_MAKE(255, 0, 0);
   togStyle.body.radius = 3;
   togStyle.text.color = LV_COLOR_MAKE(0, 164, 255);
