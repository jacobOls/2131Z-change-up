...
void disabled() {
  ...
  // std::uint32_t state = selection::gui_task_ptr->get_state();
  // std::cout << "resuming gui task " << selection::gui_task_ptr << " with
  // state " << state <<  std::endl; selection::gui_task_ptr -> resume();
  // }
  std::cout << "auton disabled" << std::endl;
}
...
void autonomous() {
  std::cout << "autonomous: gui_ptr: "
            << " in Auton " << selection::inAuton << std::endl;
  selection::execute();
