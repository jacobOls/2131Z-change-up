...
double rightDriveRamp(double RequestedPower, double Power,
                      double sidebias = 0) {
  RP = RequestedPower;
  P = Power;
  if (abs(RP) > abs(P) && RP != 0) {
    if (abs(P) < initalRamp)
      ...
      P = abs(P) + rightRampingChange + sidebias;
    P = abs(P) * sgn(RP);
  } else
    P = RP;
  return P;
...
void lEncController() {
  ...
  if (fabs(lEncCurrentValue) > fabs(right.get_position() / 100)) {
    lEncOutput = lEncOutput -
                 ((fabs(lEncCurrentValue) - fabs(right.get_position() / 100)) /
...
void rEncController() {
  ...
  if (fabs(rEncCurrentValue) > fabs(left.get_position() / 100)) {
    rEncOutput = rEncOutput -
                 ((fabs(rEncCurrentValue) - fabs(left.get_position() / 100)) /
...
void turnController() {
  ...
  turnOutput = (turn_Kp * turnErr) + (rEnc_Ki * rEncInt * rEncDt) +
               (turn_Kd * turnDer / turnDt);
